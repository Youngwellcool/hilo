...
var Holdbacks = ns.Holdbacks = Hilo.Class.create({
    ...
    constructor: function(properties){
        ...
        //障碍物的高度（包括障碍物之间的间隔）
        this.hoseHeight = 148 + this.hoseSpacingY;

        //初始化障碍的宽和高
        this.height = this.hoseHeight * this.numHoses;
        this.width = 100;//properties.height;
    ...
    startY: 0, //障碍开始的起始y轴坐标
    groundY: 0, //地面的y轴坐标

    hoseSpacingY: 0, //管子之间的水平间隔
    ...
    createHoses: function(image){
        for(var i = 0; i < this.numHoses; i++){
            ...
            this.placeHose(upHose, i);
        }           
    },

    placeHose: function(down, up, index){
        //下面障碍在y轴的最上的位置
        var downMinY = this.groundY - down.height;
        //下面障碍在y轴的最下的位置
    ...
    startMove: function(){
        ...
        //设置缓动时间
        this.moveTween.duration = (this.y - targetY) * 4;
    ...
    // calcPassThrough: function(y){
    //     var count = 0;

    //     y = -this.y + y;
    //     if(y > 0){
    //         var num = y / this.hoseWidth + 0.5 >> 0;
    //         count += num;
    //     }
